...
router.get('/violations-cycle', authenticate, async (req, res) => {
    try {
        ...
        // Get violations without associations for now (to avoid association errors)
        ...
        const formattedViolations = violations.rows.map(violation => {
            return {
                id: violation.id,
                // From Violation record (contains all needed data)
                radarId: violation.radarId,
                radarName: `Camera ${violation.cameraId || violation.radarId}`,
                location: violation.location || 'Unknown Location',
                
                // Plate Recognition Data (stored in violation)
                plateNumber: violation.plateNumber || 'UNKNOWN',
                confidence: violation.confidence || 0,
                
                // Speed Data (stored in violation)
                speed: violation.speed || 0,
                speedLimit: violation.speedLimit || 50,
                
                // Fine Calculation (from violation record)
                fineAmount: violation.fineAmount || 0,
                status: violation.status || 'pending',
                violationType: violation.violationType || 'speeding',
                
                // Timestamps
                violationTime: violation.timestamp,
                processedAt: violation.reviewedAt,
                
                // Image Data (from violation record)
                imagePath: violation.imageUrl,
                hasImage: !!violation.imageUrl,
                originalFileName: violation.originalFileName,
                
                // Processing Data
                processingMethod: violation.processingMethod,
                confirmed: violation.confirmed,
                
                // Vehicle Info
                vehicleInfo: violation.vehicleInfo,
